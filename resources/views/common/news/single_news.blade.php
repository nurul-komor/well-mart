            ...
            <div class="basis-full lg:basis-[65%]">
                ...
                <div class="flex flex-col gap-y-4 mb-4">
                    <div class="image  ring-gray-200 w-full h-120">
                        <img src="{{ asset($news->image) }}" class="object-cover h-120 w-full" alt="new image">
                    </div>
                    <ul class="flex text-xs gap-3 items-center">
                        <li class="text-red-400">{{ $news->getCategory->name }}</li>
                        <li class="select-none">.</li>
                        <li>{{ date('M d, Y') }}</li>
                        <li class="select-none">.</li>
                        <li><span class="text-gray-400">by</span> admin</li>
                    </ul>
                    <h1 class="text-xl font-bold hover:text-red-500 duration-150">{{ $news->title }}</h1>
                    <p class="text-sm font-medium text-slate-500  inline-block">{!! $news->content !!}</p>
